    ...
    public partial class AddParent : Form
    {
                ...
                case 1:
                    {
                            ...
                            tb_3.BorderColor.Equals(Color.Green) && cb_1.BackColor.Equals(Color.Green))
                        {
                            next = true;
                            addedValue = 35;
                            Parent.LastName = tb_1.Texts;
                            Parent.Name = tb_2.Texts;
                            Parent.FatherName = tb_3.Texts;
                            Parent.Sex = cb_1.SelectedItem.ToString();
                        }
                        break;
                    }
                case 2:
                       ...
                       tb_3.BorderColor.Equals(Color.Green) && tb_4.BorderColor.Equals(Color.Green))
                        {
                            ...
                            try
                            {
                                Parent.Birthday = DateTime.Parse(tb_1.Texts);
                            ...
                            Parent.Number = tb_2.Texts;
                            Parent.Mail = tb_3.Texts;
                            Parent.SecretWord = tb_4.Texts;
                        }
                        break;
                    }
                case 3:
                    {
                        if (tb_1.BorderColor.Equals(Color.Green) && tb_2.BorderColor.Equals(Color.Green)&& tb_3.BorderColor.Equals(Color.Green))
                        {
                            next = true;
                            addedValue = 30;
                            Parent.work= tb_1.Texts;
                            Parent.username = tb_2.Texts;
                            Parent.password = tb_3.Texts;
                        }
                        break;
                    }
                case 4:
                    {
                        add_resationships ar = new add_resationships(Parent.id, -1);
                        ar.ShowDialog();
                        this.Close();
                        return;
                    }
        ...
        private void update()
        {
            switch (mode)
            {
                ...
                case 3:
                    {
                        l_comment.Text = "Укажите место работы и придумайте логин и пароль";

                        tb_1.Visible = true;
                        tb_1.PlaceholderText = "Место работы";
                        tb_1.Texts = Parent.work;
                ...
                case 4:
                    {
                        if (Parent.id > 0)
                        {
                            ...
                            l_final.Text = $"Вы только что завершили регистрацию родителя {Parent.LastName} {Parent.Name[0]}. {Parent.FatherName[0]}. " +
                                $"Теперь необходимо связать этого родителя и его ребенка. " +
                                $"Если вы можете уже это сделать то нажмите на кнопку связать. Если вы не можете пока что " +
                        ...
                        else
                        {
                            ...
                            l_final.Visible = true;
                            l_final.Text = $"Нам неудалось зарегестрировать {Parent.LastName} {Parent.Name[0]}. {Parent.FatherName[0]}. " +
        ...
        private void tb_1__TextChanged(object sender, EventArgs e)
        {
            switch (mode)
            {
                ...
                case 3:
                    {
                        var r = Regex.Match(tb_1.Texts, @"^\w{2,}");
                        if (r.Success)
                        {
                            tb_1.BorderColor = Color.Green;
                            tb_1.ForeColor = Color.Green;
